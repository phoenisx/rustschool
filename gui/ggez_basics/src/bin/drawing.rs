...
impl MainState {
    fn new(ctx: &mut Context) -> GameResult<MainState> {
        ...
        let meshes = vec![build_textured_triangle(ctx)?];
        let s = MainState {
            image1,
            image2_linear,
            image2_nearest,
...
impl event::EventHandler for MainState {
    ...
    fn draw(&mut self, ctx: &mut Context) -> GameResult {
        ...
        let scale = Vector2::new(10.0, 10.0);
...
fn main() -> GameResult {
    ...
    let cb = ggez::ContextBuilder::new("Random Drawings", "ggez").add_resource_path(resource_dir);

    let (ctx, events_loop) = &mut cb.build()?;

    println!("{}", graphics::renderer_info(ctx).unwrap_or_else(|_error| String::from("GGEZ failed")));
