...
impl EventHandler for GameState {
        ...
        while timer::check_update_time(ctx, DESIRED_FPS) {
            ...
            match self.state {
                GameStates::PLAY => {
                    ...
                    if self.ball.get_left_edge() < 0.0 {
                        self.player2.increment_score();
                        self.ball.reset();
                        self.state = GameStates::START;
                    }
                    if self.ball.get_right_edge() > WINDOW_WIDTH {
    ...
    fn draw(&mut self, ctx: &mut Context) -> GameResult {
        ...
        let mut player1_score_text = CustomText::new(ctx, self.player1.get_score_string(), Some(64.0));
        let mut player2_score_text = CustomText::new(ctx, self.player2.get_score_string(), Some(64.0));
